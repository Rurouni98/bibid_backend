...
//                if (auctionStartTime.isBefore(thirtyMinutesLater)) {
                if (auctionStartTime.isBefore(fiveMinutesLater)) {
...
//                    // 경매 시작까지 5분 이상 남은 경우: 30분 전에 스케줄 등록
...
//                    log.info("30분 전 스케줄 등록: 경매 ID {}", auction.getAuctionIndex());
                    ...
                    // 경매 시작까지 5분 이상 남은 경우: 5분 전에 스케줄 등록
                    specialAuctionScheduler.scheduleChannelAllocation(
                            auction.getAuctionIndex(), auctionStartTime.minusMinutes(5)
                    );
                    log.info("5분 전 스케줄 등록: 경매 ID {}", auction.getAuctionIndex());
                }
            }
        }
...
//        );
        ...
        // 서버 재시작 시, 이미 경매가 시작된 지 10분이 지난 경매 중 아직 채널이 반납되지 않은 경매에 대해 강제 반납 스케줄링
        List<Auction> auctionsToRelease = specialAuctionRepository.findAllByAuctionTypeAndStartingLocalDateTimeBeforeAndLiveStationChannelIsNotNull(
                "실시간 경매", now.minusMinutes(10)
        );
...
//            log.info("1시간 경과로 강제 채널 반납: 경매 ID {}", auction.getAuctionIndex());
    ...
    private void rescheduleNotificationsOnStartup() {
        List<Notification> notifications = notificationRepository.findByIsSentFalse(); // 미발송 알림만 조회
        ...
        notifications.forEach(notification -> {
            if (notification.getAlertCategory() == NotificationType.AUCTION_START) {
                ...
                if (auction != null) {
                    ...
                    specialAuctionScheduler.registerAlarmForUser(auction, memberIndex); // 사용자별 알림 스케줄링 재등록
